...
public class activity_add_asignatura extends AppCompatActivity {
    ...
    public boolean validarAs(){
        ...
        if (c1.isEmpty()){
            ...
            if(inicio_lunes.getText().toString().isEmpty()||final_lunes.getText().toString().isEmpty()){
                Toast.makeText(this, "Los campos de hora no pueden quedar vacios", Toast.LENGTH_SHORT).show();
                retorno = false;
            }else{
    ...
    public boolean validarCampo(String inicio, String fin){
        ...
        if (hora_inicio > hora_final) {
            Toast.makeText(this, "La hora de inicio debe ser menor a la de final", Toast.LENGTH_SHORT).show();
            retorno = false;
